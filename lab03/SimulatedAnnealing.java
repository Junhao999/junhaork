...
import com.aim.lab03.Lab03TestFrameConfig.Schedule;
...
public class SimulatedAnnealing extends SinglePointSearchMethod {
	...
	protected void runMainLoop() {
		...
		double f0 = problem.getObjectiveFunctionValue(BACKUP_SOLUTION_INDEX);
		
		double temp = this.oCoolingSchedule.getCurrentTemperature();
		
		double Fbest = problem.getObjectiveFunctionValue(CURRENT_SOLUTION_INDEX);
		
		problem.bitFlip(CURRENT_SOLUTION_INDEX+1);
		if(Fbest<=f0) {//CURRENT_SOLUTION_INDEX is currently BEST
			      //BACKUP_SOLUTION_INDEX is original solution
		...
		f0=Fbest;
			...
			if(x < 0 || Math.exp(-x/temp)<Math.random()) {
